		...
		<tbody>
			<?php foreach ($this->model->ocurrences as $ocurrence): ?>
				<tr>
					...
					<td>
					<?php if ($this->logged_in): ?>
						<a href="<?=$this->model->edit_url . $ocurrence['Id']?>" class="button success small cad icon-edit"></a>
					<?php endif ?>
						<a href="<?=$this->model->visualize_url . $ocurrence['Id']?>" class="button info small cad icon-visu"></a>
					</td>
				</tr>
			<?php endforeach ?>
		</tbody>
